...
// PUT/DELETE route paths contain issueId param
...
// Creates new issue entry for comic book series
issuesRouter.post('/', (req, res, next) => {
  ...
  db.get(artistSql, artistValues, (error, artist) => {
    if (error) {
      ...
      const issuesSql = 'INSERT INTO Issue (name, issue_number, publication_date, artist_id, series_id) ' +
      ...
      db.run(issuesSql, issuesValues, function(error) {
        if (error) {
          next(error);
        } else {
...
// Updates issue entry for comic book series
...
// Deletes issue entry from comic book series
